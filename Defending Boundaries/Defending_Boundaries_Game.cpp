...
int main()
{
	...
	for (int i = 0;i < sScrW * sScrH;i++)wpScreen[i] = L' ';
	...
	while (!Game_Vairables::GameTime.IsTimeOver())
	{
		...
		if (Game_Vairables::Startit)Game_Vairables::GameTime.SubtractFrom_FSeconds(Game_Vairables::SecPassed.count());
...
void GameModule_Inialization(wchar_t* Screen)
{
	...
	for (int i = sScrW * 3;i < sScrW * 3 + sScrW;i++)Screen[i] = L'=';
